              ...
              <div class="row">
                <div class="col-md-6">
                  <table class="table table-bordered">
                    ...
                    <tr><th>Merit Rank:</th><td><span class="label label-primary">#${allot.meritRank || '--'}</span></td></tr>
                    <tr><th>Academic Score:</th><td>${allot.aggregate ? Number(allot.aggregate).toFixed(2) + '%' : '--'}</td></tr>
                  </table>
                </div>
                <div class="col-md-6">
                  <table class="table table-bordered">
                    <tr><th width="45%">Original Category:</th><td><strong>${allot.category || '--'}</strong></td></tr>
                    <tr><th>Common Category:</th><td>${allot.commonCategory || '--'}</td></tr>
                    <tr><th>Allotted Quota:</th><td><span class="label ${allot.allotmentRound === 'SPOT' ? 'label-primary' : (allot.isConverted ? 'label-warning' : 'label-default')}" style="${allot.allotmentRound === 'SPOT' ? 'background-color: #7c3aed;' : ''}">${allot.allotmentCategory || 'OPEN'} ${allot.allotmentRound === 'SPOT' ? '(Common Pool)' : (allot.isConverted ? '(Converted)' : '')}</span></td></tr>
                    <tr><th>Allocation Mode:</th><td>${allot.allotmentRound === 'SPOT' ? '<span class="label" style="background-color: #7c3aed; color: #fff;"><i class="glyphicon glyphicon-star"></i> Stage 3 Common Pool Spot Round</span>' : (allot.isConverted ? '<span class="label label-warning"><i class="glyphicon glyphicon-random"></i> Converted from Reserved to OPEN</span>' : '<span class="label label-info">Standard Quota Allotment</span>')}</td></tr>
                  </table>
                </div>
              </div>
